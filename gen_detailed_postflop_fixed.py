#!/usr/bin/env python3
"""
为所有翻后题（flop/turn/river）生成详细推理 - 修复版
...
def evaluate_hand_on_board(hand_name, board):
    ...
    has_flush_draw = has_suit and any(s == hand_name[0] for s in board_suits)  # 简化
...
def make_detailed_postflop_reasoning(hand_name, position, street, board, optimal_action, freq, equity):
    ...
    # 评估手牌强度
    hand_strength, hand_desc = evaluate_hand_on_board(hand_name, board)
...
print("- 翻后题：包含牌面纹理分析、手牌强度评估、GTO频率依据、具体策略、翻后计划")
