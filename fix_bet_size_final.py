...
for match in matches:
    ...
    if board_type == 'dry':
        ...
        new_reasoning = re.sub(r'OOP应该更激进，用check-raise保护范围。湿润牌面check范围应该平衡：包含价值牌（慢打）和诈唬牌。', 
                              'OOP（无位置）应该check更多，用check-raise保护范围。强牌可以慢打，弱牌可以诈唬。', 
                              new_reasoning)
        ...
        # 替换策略部分
        new_reasoning = re.sub(r'干燥牌面（如A-high、K-high）有利于翻前加注者（IP），因为范围优势明显。', 
    ...
    if new_reasoning != reasoning_text:
        ...
        # 构建新的完整匹配
        new_scenario_text = scenario_text.replace(reasoning_text, new_reasoning_escaped)
